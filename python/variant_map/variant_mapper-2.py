...
WEIGHT_METH_ASSOCIATION = 0.6       # Now higher weight for methylation association (we want them to be unmethylated)
WEIGHT_UNMETH_EXCLUSIVITY = 0.4   # Lower weight for unmethylation exclusivity
    ...
    def write_results(self, variants: List[Dict]) -> bool:
        ...
        try:
            ...
            headers = [
                ...
                'meth_samples', 'unmeth_samples', 'no_data_samples', 'no_meth_samples', 'no_var_meth_samples', 'no_var_unmeth_samples',
            ...
            with open(meth_file, 'w', newline='') as f:
                ...
                for var in hap_variants:  # No need to sort here, we'll sort later
                    ...
                    writer.writerow([
                        ...
                        str(var['variant']['ref']),
                        str(var['variant']['alt']),
                        f"{var['methylation_score']:.3f}",  # Include methylation_score
                        f"{var['unmethylation_score']:.3f}",  # Include unmethylation_score
                        f"{var['p_value']:.2e}",
