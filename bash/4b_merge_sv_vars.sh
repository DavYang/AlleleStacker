...
INT_VCF_2="${OUT_DIR}/merged_phased_SVs_GRCh38.filtered.renamed.vcf.gz"
FINAL_VCF="${OUT_DIR}/merged_phased_SVs_GRCh38.qc.vcf.gz"
...
bcftools filter -i 'FILTER="PASS" && DP>=10' "$MERGED_VCF" -O z -o "$INT_VCF"
tabix -p vcf "$INT_VCF"

bcftools annotate --set-id '%CHROM:%POS:%REF:%ALT:%SVANN' "$INT_VCF" -O z -o "$INT_VCF_2"
