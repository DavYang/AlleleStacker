...
def is_cpg_affected_by_variant(cpg_pos: int, var_pos: int, ref: str, alt: str) -> bool:
    """Check if CpG is directly disrupted by variant"""
    # Convert to 0-based coordinates
    var_pos = var_pos - 1
    ref_len = len(ref)
    # Check if variant overlaps CpG position
    return var_pos <= cpg_pos <= var_pos + ref_len - 1
    ...
    with pysam.FastaFile(str(ref_fasta_path)) as fasta:
        for var in variant_data_list:
            ...
            try:
                ...
                alt_seq = ref_seq[:2] + var.alt + ref_seq[2 + len(var.ref):]
                ...
                for i in range(len(ref_seq)-1):
                    if ref_seq[i:i+2] == 'CG':
                        # Validate reference CpG
                        if fasta.fetch(var.chrom, start + i, start + i + 2).upper() == 'CG':
                ...
                if var.gt == (1,1):  # Homozygous ALT
                    ...
                    if destroyed:
                        local_effects[alt_hap].ref_destroyed.update(
                            pos for pos in destroyed 
                            if is_cpg_affected_by_variant(pos, variant_pos, var.ref, var.alt)
                        )
...
def main():
    ...
    parser.add_argument('--vcf', required=True, help='Input VCF directory')
